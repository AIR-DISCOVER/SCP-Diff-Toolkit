...
class ADE20KDataset(Dataset):
    ...
    def __init__(self):
        ...
        for root, _, files in os.walk(osp.join(self.BASE, 'images', 'training')):
            for file in files:
                self.data.append(osp.join(self.BASE, 'images', 'training', file))
        
        for root, _, files in os.walk(osp.join(self.BASE, 'annotations', 'training')):
            for file in files:
                self.labels.append(osp.join(self.BASE, 'annotations', 'training', file))
